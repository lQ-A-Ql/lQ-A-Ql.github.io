...
export function BlogCard({
  ...
  const content = (
    <motion.article
      className="group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-[1.9rem] border border-white/[0.03] bg-transparent transition-all duration-500 hover:-translate-y-2 hover:border-primary/[0.08]"
    ...
    >
      ...
      {imageUrl && (
        <div className="relative aspect-video overflow-hidden">
          ...
          <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(10,8,18,0.02)_0%,rgba(10,8,18,0.08)_28%,rgba(10,8,18,0.58)_100%)]" />

          <div className="absolute top-4 left-4">
            <span className="rounded-full border border-primary/20 bg-transparent px-3 py-1 text-[11px] font-medium tracking-[0.14em] text-primary">
              {category}
            </span>
          </div>
          <span className="absolute top-4 right-4 text-[11px] uppercase tracking-[0.24em] text-white/38">{entryCode}</span>
        </div>
      )}
      ...
      <div className="relative flex h-full flex-col p-6">
        {!imageUrl && (
          <div className="mb-5 flex items-center justify-between gap-3 pb-4">
            <span className="inline-flex rounded-full border border-primary/18 bg-transparent px-3 py-1 text-[11px] font-medium tracking-[0.14em] text-primary">
              {category}
            </span>
            <span className="text-[11px] uppercase tracking-[0.24em] text-white/34">{entryCode}</span>
          </div>
        )}
        ...
        <p className="mb-6 line-clamp-4 text-sm leading-7 text-white/80 [text-shadow:0_1px_8px_rgba(0,0,0,0.08)]">
          {excerpt}
        </p>
